...
public interface WorkspaceRepository extends JpaRepository<Workspace, Long> {
    ...
    @Query(value = "SELECT CASE WHEN COUNT(*) > 0 THEN true ELSE false END FROM workspaces w where w.user_id = :userId and w.workspace_name= :workspaceName", nativeQuery = true)
